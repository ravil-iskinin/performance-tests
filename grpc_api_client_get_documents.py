from clients.grpc.gateway.users.client import build_users_gateway_grpc_client
from clients.grpc.gateway.accounts.client import build_accounts_gateway_grpc_client
from clients.grpc.gateway.documents.client import build_documents_gateway_grpc_client

users_gateway_client = build_users_gateway_grpc_client()
accounts_gateway_client = build_accounts_gateway_grpc_client()
documents_gateway_client = build_documents_gateway_grpc_client()

# Создаем пользователя
create_user_response = users_gateway_client.create_user()
print('Create user data:', create_user_response)

# # Открыть кредитный счет
open_debit_card_account_response = accounts_gateway_client.open_debit_card_account(user_id=create_user_response.user.id)
print('Open credit card:', open_debit_card_account_response)

# Получить документ тарифа
document_tariff_response = documents_gateway_client.get_tariff_document(account_id=open_debit_card_account_response.account.id)
print('Get tariff document:', document_tariff_response)

# Получить документ контракта
document_contract_response = documents_gateway_client.get_contract_document(account_id=open_debit_card_account_response.account.id)
print('Get contract document:', document_contract_response)
